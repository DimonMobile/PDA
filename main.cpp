...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        Fst fst = {
                Fst::Vertex( _(1, Edge::create(L's')) ).setAction( []() {std::wcout << L"First" << std::endl;} )                                    // 0
            ,   Fst::Vertex( _(2, Edge::create(L'q')) )                                    // 1
            ,   Fst::Vertex( _(3, Edge::create(L'r')) )                // 2
            ,   Fst::Vertex( _(4, Edge::create(L't')) ).setAction( [](){std::wcout << L"Курлык" << std::endl;} )                                    // 3
            ,   Fst::Vertex( _(5, Edge::create(L'(')) )                                    // 4
            ,   Fst::Vertex( _(6, alphaEdge), _(5, alphaEdge))               // 5
            ,   Fst::Vertex( _(7, Edge::create(L')')) )                                   // 6
            ,   Fst::Vertex().setAction( [](){std::wcout << L"Full" << std::endl; } )                                                                           // 7
        };
        std::wcout << "***FinalStepTranducer: " << fst.execute(L"sqrt(выф)") << std::endl;
    }
